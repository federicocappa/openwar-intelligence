...
Fonti (nessuna chiave). Tre reti di ricevitori indipendenti, unite per hex:
  adsb.lol       /v2/mil   (ODbL)                 primaria
  adsb.fi        /v2/mil   (uso non commerciale)  riempie i buchi di copertura, porta desc+ownOp
  airplanes.live /v2/mil   (uso non commerciale)  idem, spesso irraggiungibile
  adsb.lol /v2/point/lat/lon/250 sui teatri: tutto il traffico, qui si pescano i militari
  che i database non marcano (Russia, Cina, Iran...) per callsign, matricola, tipo, hex.
E' la stessa catena usata da World Monitor (adsb.lol primaria + airplanes.live/adsb.fi gap-fill).
Output:
...
# Prefissi di matricola (civili e militari) -> nazione, usati quando il blocco hex non basta.
REG_PREFIX = [
    ("RF-", "RUS"), ("RA-", "RUS"), ("N", "USA"), ("G-", "UK"), ("ZZ", "UK"), ("ZM", "UK"), ("ZK", "UK"), ("ZH", "UK"), ("ZJ", "UK"),
    ("D-", "DEU"), ("F-", "FRA"), ("I-", "ITA"), ("MM", "ITA"), ("EC-", "ESP"), ("T.", "ESP"), ("PH-", "NLD"), ("OO-", "BEL"),
    ("SP-", "POL"), ("SE-", "SWE"), ("LN-", "NOR"), ("OY-", "DNK"), ("OH-", "FIN"), ("SX-", "GRC"), ("TC-", "TUR"), ("HB-", "CHE"),
    ("OE-", "AUT"), ("CS-", "PRT"), ("OK-", "CZE"), ("HA-", "HUN"), ("YR-", "ROU"), ("C-", "CAN"), ("VH-", "AUS"), ("A6-", "ARE"),
    ("HZ-", "SAU"), ("A7-", "QAT"), ("9K-", "KWT"), ("SU-", "EGY"), ("4X-", "ISR"), ("EP-", "IRN"), ("AP-", "PAK"), ("VT-", "IND"),
    ("JA", "JPN"), ("HL", "KOR"), ("B-", "CHN"), ("PR-", "BRA"), ("PT-", "BRA"), ("FAB", "BRA"), ("UR-", "UKR"), ("9V-", "SGP"),
    ("PK-", "IDN"), ("RP-", "PHL"), ("ZS-", "ZAF"), ("ET-", "ETH"), ("5Y-", "KEN"), ("CN-", "MAR"), ("7T-", "DZA"),
]
...
def to_row(a, src, ts, run_id):
    ...
    return dict(
        ...
        label=LABELS.get(t) or (a.get("desc") or "").title() or t or "Military",
        op=(a.get("ownOp") or "").title(),
    )
...
def collect():
    ...
    found, stats = {}, {"mil_feed": 0, "feeds": {}, "hotspots": 0, "by_reason": {}}
    for name, url in MIL_FEEDS:
        d = http_json(url, retries=1)
        n = 0
        for a in (d or {}).get("ac", []) or []:
            ...
            if hx in found:
                # arricchisci con desc/ownOp se la prima fonte non li aveva
                if a.get("ownOp") and not found[hx].get("op"):
                    found[hx]["op"] = a["ownOp"].title()
                continue
            found[hx] = to_row(a, f"{name}/mil", ts, run_id)
            n += 1
        stats["feeds"][name] = n if d else "down"
        stats["mil_feed"] += n
...
def main():
    ...
    dash = [dict(icao=r["hex"], call=r["callsign"] or r["hex"].upper(), lat=r["lat"], lng=r["lon"], alt=r["alt"],
                 ...
                 model=r["type_code"], reg=r["reg"], op=r.get("op", ""), source="owi-live", zone=r["zone"]) for r in rows]
    nations = {}
    for r in rows:
    ...
    print(f"[live] {len(dash)} velivoli militari (feed {stats['mil_feed']} {stats['feeds']}, hotspot +{stats['hotspots']} {stats['by_reason']}); "
