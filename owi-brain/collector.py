...
SAMPLES = int(os.environ.get("OWI_SAMPLES", "2"))  # il feed live (ogni 10 min) e' la fonte principale
INTERVAL_MIN = float(os.environ.get("OWI_INTERVAL_MIN", "5"))


def http_json(url, timeout=40, retries=2, backoff=(3, 6, 12), wait_429=10):
    """GET JSON con retry breve. I 429 (GDELT limita gli IP condivisi di GitHub) NON devono
    allungare il run: il 04-13/09 nove run sono stati uccisi dal timeout per questo."""
    last = None
    for i in range(retries + 1):
        try:
            ...
            last = e
            if i < retries:
                time.sleep(wait_429 if e.code == 429 else backoff[min(i, len(backoff) - 1)])
...
def collect_live_samples(days=3):
...
GDELT_BUDGET_S = int(os.environ.get("OWI_GDELT_BUDGET_S", "240"))
...
def collect_gdelt(days_back=3):
    ...
    Budget di tempo fisso: se GDELT non risponde, si salta e il run va avanti."""
    fetched = store.utcnow().strftime("%Y-%m-%dT%H:%MZ")
    n = 0
    deadline = time.time() + GDELT_BUDGET_S
    for z in ZONES:
        if time.time() > deadline:
            print(f"  GDELT: budget di {GDELT_BUDGET_S}s esaurito, salto le zone restanti")
            break
        q = urllib.parse.urlencode(dict(query=z["gdelt"], mode="timelinevol", timespan=f"{days_back + 1}d", format="json"))
        d = http_json(f"{GDELT_URL}?{q}", timeout=30, retries=1)
        rows = []
        try:
        ...
        if rows:
            ...
            n += len(rows)
        time.sleep(6)  # cortesia verso GDELT
