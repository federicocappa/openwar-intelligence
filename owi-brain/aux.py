...
def fetch(url, timeout=60, retries=2):
    # i runner GitHub a volte rispondono "Network is unreachable" (IPv6 senza rotta): si riprova dopo una pausa
    last = None
...
# (query, categoria, nazione, ruolo, filtro-nome). La ricerca NAME di CelesTrak e' per sottostringa
# (RISAT prende anche MARISAT/BRISAT, COSMO prende 2.859 COSMOS): il filtro tiene solo i nomi giusti.
SAT_QUERIES = [
    ("NAME=SBIRS", "EW", "USA", "early warning IR", r"SBIRS"),
    ("NAME=DSP", "EW", "USA", "early warning IR", r"DSP (F?1[4-9]|F?2\d)"),
    ("NAME=WGS", "COMMS", "USA", "comms militari", r"^WGS"),
    ("NAME=AEHF", "COMMS", "USA", "comms protette", r"AEHF"),
    ("NAME=MUOS", "COMMS", "USA", "comms navali", r"^MUOS"),
    ("NAME=SBSS", "SSA", "USA", "sorveglianza spaziale", r"^SBSS"),
    ("NAME=WORLDVIEW", "ISR", "USA", "ottico commerciale (Maxar, uso NGA)", r"^WORLDVIEW"),
    ("NAME=LEGION", "ISR", "USA", "ottico commerciale (Maxar Legion)", r"LEGION"),
    ("NAME=SKYNET", "COMMS", "GBR", "comms militari", r"^SKYNET"),
    ("NAME=SYRACUSE", "COMMS", "FRA", "comms militari", r"^SYRACUSE"),
    ("NAME=HELIOS", "ISR", "FRA", "ottico militare", r"^HELIOS"),
    ("NAME=PLEIADES", "ISR", "FRA", "ottico dual-use", r"^PLEIADES"),
    ("NAME=SAR-LUPE", "SAR", "DEU", "radar militare", r"^SAR-LUPE"),
    ("NAME=TERRASAR", "SAR", "DEU", "radar dual-use", r"^TERRASAR"),
    ("NAME=TANDEM", "SAR", "DEU", "radar dual-use", r"^TANDEM"),
    ("NAME=COSMO-SKYMED", "SAR", "ITA", "radar dual-use", r"^COSMO-SKYMED"),
    ("NAME=CSG-", "SAR", "ITA", "radar dual-use (CSG)", r"^CSG-"),
    ("NAME=SENTINEL-1", "SAR", "EU", "radar Copernicus", r"^SENTINEL-1"),
    ("NAME=ICEYE", "SAR", "FIN", "radar commerciale (uso UKR)", r"^ICEYE"),
    ("NAME=CAPELLA", "SAR", "USA", "radar commerciale", r"^CAPELLA"),
    ("NAME=YAOGAN", "ISR", "CHN", "ISR/ELINT militare", r"^YAOGAN"),
    ("NAME=GAOFEN", "ISR", "CHN", "ottico/radar dual-use", r"^GAOFEN"),
    ("NAME=TJS", "SIGINT", "CHN", "SIGINT/EW GEO", r"^TJS-\d+$"),
    ("NAME=SHIJIAN", "SSA", "CHN", "sperimentale/SSA", r"^SHIJIAN-(1[6-9]|2\d)"),
    ("NAME=KONDOR", "SAR", "RUS", "radar militare", r"^KONDOR"),
    ("NAME=KOMPSAT", "ISR", "KOR", "ottico/radar dual-use", r"KOMPSAT-[2-7]"),
    ("NAME=GOKTURK", "ISR", "TUR", "ottico militare", r"^GOKTURK"),
    ("NAME=CARTOSAT", "ISR", "IND", "ottico dual-use", r"^CARTOSAT-[23]"),
    ("NAME=RISAT", "SAR", "IND", "radar militare", r"^RISAT|^EOS-0[49]"),
    ("NAME=EMISAT", "SIGINT", "IND", "ELINT", r"^EMISAT"),
    ("GROUP=military", "MIL", None, "vario", None),  # per ultimo: le query specifiche hanno la precedenza
]
SAT_MAX = 480
SAT_PER_QUERY = 60  # Yaogan da solo ha 180 oggetti: senza tetto per query schiaccia SIGINT/EW/comms


def build_satellites():
    seen, sats, errors = set(), [], []
    for q, cat, nation, role, name_re in SAT_QUERIES:
        try:
        ...
        for i in range(0, len(lines) - 2, 3):
            ...
            if name_re and not re.search(name_re, name):
                continue
            if re.search(r"\bDEB\b|R/B|\bAKM\b|PLAT\b|\bSTAGE\b", name):
                continue  # detriti, stadi e motori: non sono sensori
            norad = l1[2:7].strip()
            if norad in seen:
                continue
            seen.add(norad)
